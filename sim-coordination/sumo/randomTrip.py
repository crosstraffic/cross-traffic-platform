...
def get_options(args=None):
    ...
    op.add_argument("--op-flows", dest="op_flows", category="flow", default=0, type=int,
                    help="generates INT opposite flows that together output vehicles with the specified period")
...
class DestinedTripGenerator:
    ...
    def get_trip(self, min_distance, max_distance, maxtries=100, junctionTaz=False, min_dist_fringe=None, src_index=0, sink_index=-1, via_index=0):
...
def buildTripGenerator(net, options):
    ...
    try:
        ...
        via_generator = DestinedEdgeGenerator(
            net, get_prob_fun(options, None, None, 1))
    ...
    #     source_generator, sink_generator, via_generator, options.intermediate, options.pedestrians, src_index, sink_index, via_index)
    return DestinedTripGenerator(
...
def main(options):
    ...
    op_trip_generator = buildTripGenerator(options.net, options)
    idx = 0
    ...
    def generate_origin_destination(trip_generator, options, src_index, sink_index, via_index):
        source_edge, sink_edge, intermediate = trip_generator.get_trip(
            options.min_distance, options.max_distance, options.maxtries,
            options.junctionTaz, options.min_dist_fringe, src_index, sink_index, via_index)
        return source_edge, sink_edge, intermediate
        ...
        if options.binomial:
            ...
            if is_op:
                fouttrips.write(('    <flow id="%s" begin="%s" end="%s" period="%s"%s/>\n') % (
                    label, departureTime, arrivalTime, intIfPossible(period * options.op_flows), combined_attrs))
            else:
                fouttrips.write(('    <flow id="%s" begin="%s" end="%s" period="%s"%s/>\n') % (
    ...
    def generate_one(idx, departureTime, arrivalTime, period, origin, destination, intermediate, timeIdx=None, is_op=False):
        try:
